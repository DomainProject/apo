...
bool clingo_model_to_strings(clingo_model_t const *model, size_t *as_n, char **as[])
{
	...
	for(it = atoms, ie = atoms + atoms_n; it != ie; ++it, ++i) {
		...
		if(!clingo_symbol_to_string(*it, as_tmp[i], n)) {
			goto error;
		}
	}
...
bool init_clingo(char const *program, int argc, char const **argv, clingo_ctx **cctx)
{
	...
	if(!clingo_control_new(argv, argc, NULL, NULL, 20, &cctx_tmp->ctl) != 0) {
		goto error;
	}

	// add a logic program to the base part
	if(!clingo_control_add(cctx_tmp->ctl, "base", NULL, 0, program)) {
		goto error;
	}

	// ground the base part
	if(!clingo_control_ground(cctx_tmp->ctl, cctx_tmp->parts, 1, NULL, NULL)) {
		goto error;
	}
...
bool get_model(clingo_ctx *cctx, clingo_model_t const **model)
{
	...
	if(!clingo_solve_handle_model(cctx->handle, model)) {
		goto error;
	}
...
bool free_clingo(clingo_ctx *cctx)
{
	...
	if(!clingo_solve_handle_get(cctx->handle, &solve_ret)) {
		goto error;
	}

	// free the solve handle
	if(!clingo_solve_handle_close(cctx->handle)) {
		goto error;
	}

	// free the control structure created with clingo_control_new
	if(cctx->ctl) {
		clingo_control_free(cctx->ctl);
	}
