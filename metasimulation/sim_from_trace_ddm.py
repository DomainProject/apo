...
def check_and_install_new_binding(cu_units_data):
    global assignment
    binding = ddm_poll()
    if binding is None: return False
    sim_binding = convert_ddm_assignment_to_sim_assingment(binding)
    assignment = sim_binding
...
def simulation():
    ...
    while is_there_any_pending_evt():
        ...
        if evt_t == EVT.TIME_WINDOW and not rebalance_in_progress:
            start_rebalance(cu_units_data, wct_ts)
        ...
        if rebalance_in_progress:
            if check_and_install_new_binding(cu_units_data):
                rebalance_in_progress = False
...
def start_rebalance(cu_units_data, wct_ts):
    global min_vt
    ...
    ddm_optimize(num_actors, ddm_prepare_actor_matrix(actor_matrix),
                 get_events_count_vector_in_next_window(wct_ts + time_window_size, num_actors), len(build_cunits()), get_capacity_vector())
